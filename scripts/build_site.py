...
<section id="live-bench" class="interactive-bench" aria-label="Live Benchmark">
  ...
  {
    "role": "system",
    "content": "You are an automated traffic router. Classify user input into a JSON object: {\\"threat\\": boolean, \\"action\\": \\"BLOCK\\"|\\"ESCALATE\\"|\\"ROUTE_TECH\\"|\\"ROUTE_SPAM\\", \\"severity\\": \\"LOW\\"|\\"MEDIUM\\"|\\"HIGH\\"|\\"CRITICAL\\", \\"reason\\": string}. Respond ONLY with raw compact JSON, no markdown block."
  },
  {
    "role": "user",
    "content": "&lt;待测输入文本&gt;"
    ...
    <div class="bench-col jev-col">
      ...
      <div class="bench-latency-box">
        <div class="bench-latency-num">
          ...
          <span id="jev-latency-type" class="bench-latency-type lang-zh">OpenRouter 响应</span>
          <span id="jev-latency-type-en" class="bench-latency-type lang-en" style="display:none">OpenRouter API</span>
        </div>
      ...
      <div class="bench-cost-tag">
        ...
        <strong id="jev-cost-val">~$0.000006</strong>
      </div>
    </div>
  ...
  "reason": "SQL injection tautology detected."
      ...
      <div class="bench-cost-tag">
        <span class="lang-zh">单次调用成本</span>
        <span class="lang-en" style="display:none">Cost / Call</span>
        <strong id="llm-cost-val">~$0.000025</strong>
      </div>
    </div>
  </div>
  ...
  <div class="bench-summary-bar">
    ...
    <div>
      ...
      <span class="lang-zh" id="summary-cost-zh">单次立省 ~$0.000019 (降 76.5%)</span>
      <span class="lang-en" id="summary-cost-en" style="display:none">Save ~$0.000019 / call (-76.5%)</span>
    </div>
    <div>
...
var BENCH_DATA = {
  sql_injection: {
    ...
    summary: {
      ...
      costZh: "单次立省 ~$0.000019 (降 76.5%)",
      costEn: "Save ~$0.000019 / call (-76.5%)"
    }
  },
  angry_customer: {
    ...
    summary: {
      ...
      costZh: "单次立省 ~$0.000018 (降 75.0%)",
      costEn: "Save ~$0.000018 / call (-75.0%)"
    }
  },
  tech_support: {
    ...
    summary: {
      ...
      costZh: "单次立省 ~$0.000020 (降 76.9%)",
      costEn: "Save ~$0.000020 / call (-76.9%)"
    }
  },
  spam_promo: {
    ...
    summary: {
      ...
      costZh: "单次立省 ~$0.000017 (降 73.9%)",
      costEn: "Save ~$0.000017 / call (-73.9%)"
    }
  }
};
...
async function runBenchmark() {
  ...
  var handleLiveJev = async function() {
    ...
    if (res && res.mode === 'live' && res.answers) {
      jevLiveSuccess = true;
      var realJevMs = res.latency_ms || 140;
      if (jevTimerEl) jevTimerEl.textContent = realJevMs;
      ...
      var pTokens = (res.usage && res.usage.prompt_tokens) ? res.usage.prompt_tokens : 80;
      var jevCost = (pTokens * 0.075) / 1000000;
      if (jevCostVal) jevCostVal.textContent = '$' + jevCost.toFixed(6);

      return { latency: realJevMs, e2e: e2e, cost: jevCost };
    } else {
      // Fallback
      ...
      if (jevScoreVal) jevScoreVal.textContent = currentLang === 'zh' ? matchedData.jev.scoreZh : matchedData.jev.scoreEn;
      if (jevCostVal) jevCostVal.textContent = matchedData.jev.cost;
      return { latency: pureFall, e2e: 1180, cost: 0.000006 };
    }
  };
  ...
  var handleLiveLlm = async function() {
      ...
      if (res && res.ok && (res.headers.get('content-type') || '').indexOf('text/event-stream') >= 0) {
        ...
        while (true) {
          ...
          if (!firstTokenTime) {
            ...
            var localTtft = Math.round(firstTokenTime - startTime);
            if (llmTimerEl) llmTimerEl.textContent = localTtft.toLocaleString();
          }
          ...
          for (var i = 0; i < lines.length; i++) {
            ...
            if (line.indexOf('data: ') === 0) {
              try {
                ...
                if (parsedChunk._upstream_ttft) {
                  serverTtft = parsedChunk._upstream_ttft;
                }
                if (parsedChunk.usage) {
                  realUsage = parsedChunk.usage;
                }
        ...
        var finalTtft = serverTtft || (firstTokenTime ? Math.round(firstTokenTime - startTime) : 810);

        llmLiveSuccess = true;
        if (llmTimerEl) llmTimerEl.textContent = finalTtft.toLocaleString();

        var outTokens = realUsage ? realUsage.completion_tokens : 66;
        var inTokens = realUsage ? realUsage.prompt_tokens : 85;
        var realLlmCost = (inTokens * 0.075 + outTokens * 0.30) / 1000000;
        ...
        if (llmCostVal) llmCostVal.textContent = '$' + realLlmCost.toFixed(6);
        return { ttft: finalTtft, totalLlm: totalLlm, cost: realLlmCost };
      }
    } catch (e) {}
    ...
    if (llmSeverityVal) llmSeverityVal.textContent = currentLang === 'zh' ? matchedData.llm.severityZh : matchedData.llm.severityEn;
    if (llmCostVal) llmCostVal.textContent = matchedData.llm.cost;
    return { ttft: targetTtft, totalLlm: 1160, cost: 0.000025 };
  };
  ...
  var diffCost = Math.max(0, lRes.cost - jRes.cost);
  var savePct = Math.round((diffCost / (lRes.cost || 1)) * 1000) / 10;
  if (summaryCostZh) summaryCostZh.textContent = '单次立省 ~$' + diffCost.toFixed(6) + ' (降 ' + savePct + '%)';
  if (summaryCostEn) summaryCostEn.textContent = 'Save ~$' + diffCost.toFixed(6) + ' / call (-' + savePct + '%)';
