...
@media (max-width: 760px) {
  ...
  .bench-summary-bar { grid-template-columns: 1fr; gap: 6px; }
}
"""
...
<section id="live-bench" class="interactive-bench" aria-label="Live Benchmark">
  ...
  <p class="bench-desc lang-zh">
    为什么要做这个对比？在线上生产链路（安全拦截、工单升级、意图路由）中，任务本质是确定性的状态判别。传统大模型（如 Gemini 2.5 Flash-Lite）必须经历首字推演与逐字解码，产生可感知的延迟与累加的总账单。Jev 采用单步前向推导直接输出原生类型。点击下方 4 组生产场景，直观对比响应时延与百万次调用成本：
  </p>
  <p class="bench-desc lang-en" style="display:none">
    Why this benchmark? In production pipelines (guardrails, routing, classification), tasks only require deterministic state verdicts. Autoregressive LLMs (like Gemini 2.5 Flash-Lite) require TTFT and token decoding loops, incurring continuous latency and high total bills. Jev uses a single forward pass returning native typed values. Click the 4 presets below to compare latency and total cost per 1M calls:
  </p>
  ...
  <div class="bench-grid">
    ...
    <div class="bench-col jev-col">
      ...
      <div class="bench-latency-box">
        <div class="bench-latency-num">
          ...
          <span id="jev-latency-type" class="bench-latency-type lang-zh">纯模型推导</span>
        ...
        <div class="bench-meta-tag green">
          <span>●</span>
          <span class="lang-zh" id="jev-meta-zh">单步矩阵直出 · 零采样等待</span>
          <span class="lang-en" id="jev-meta-en" style="display:none">Matrix Forward · Zero Sampling Wait</span>
        </div>
      </div>
      <div class="bench-verdicts">
        <div class="verdict-row">
          <div class="verdict-label-row">
            ...
            <span id="jev-noul-val" class="verdict-val">高危拦截 (98.4%)</span>
          </div>
        ...
        <div class="verdict-row">
          <div class="verdict-label-row">
            ...
            <span id="jev-score-val" class="verdict-val">4 / 4 · 致命风险 (P0)</span>
          </div>
        </div>
      </div>
      <div class="bench-cost-tag">
        <span class="lang-zh">百万次调用总成本</span>
        <span class="lang-en" style="display:none">Cost / 1M Calls</span>
        <strong id="jev-cost-val">~$6.30</strong>
      </div>
    </div>
    ...
    <div class="bench-col llm-col">
      <div class="bench-col-head">
        ...
        <div>
          <span class="bench-col-type lang-zh">System 2 · 逐字解码推演</span>
  ...
  "reason": "SQL injection tautology detected."
      ...
      <div class="bench-cost-tag">
        <span class="lang-zh">百万次调用总成本</span>
        <span class="lang-en" style="display:none">Cost / 1M Calls</span>
        <strong id="llm-cost-val">~$24.80</strong>
      </div>
    </div>
  </div>
  ...
  <div class="bench-summary-bar">
    <div>
      ...
      <span id="summary-speedup">快 24.3x (28ms vs 680ms 首字)</span>
    </div>
    <div>
      <strong class="lang-zh">百万次总成本：</strong>
      <strong class="lang-en" style="display:none">Cost / 1M: </strong>
      <span class="lang-zh" id="summary-cost-zh">~$6.30 (Jev) vs ~$24.80 (LLM) · 省 75%</span>
      <span class="lang-en" id="summary-cost-en" style="display:none">~$6.30 (Jev) vs ~$24.80 (LLM) · 75% Cheaper</span>
    </div>
    <div>
      <strong class="lang-zh">数据协议契约：</strong>
      <strong class="lang-en" style="display:none">Data Contract: </strong>
      <span class="lang-zh">原生强类型 (零格式幻觉) vs JSON 文本 (需后置反序列化)</span>
      <span class="lang-en" style="display:none">Native Types (Zero Drift) vs JSON String (Parse Required)</span>
    </div>
    <div>
...
var BENCH_DATA = {
  sql_injection: {
    ...
    summary: {
      speedupZh: "快 24.3x (28ms vs 680ms 首字)",
      speedupEn: "24.3x faster (28ms vs 680ms TTFT)",
      costZh: "~$6.30 (Jev) vs ~$24.80 (LLM) · 省 75%",
      costEn: "~$6.30 (Jev) vs ~$24.80 (LLM) · 75% Cheaper"
    }
  },
  angry_customer: {
    ...
    summary: {
      speedupZh: "快 20.3x (32ms vs 650ms 首字)",
      speedupEn: "20.3x faster (32ms vs 650ms TTFT)",
      costZh: "~$6.30 (Jev) vs ~$25.40 (LLM) · 省 75%",
      costEn: "~$6.30 (Jev) vs ~$25.40 (LLM) · 75% Cheaper"
    }
  },
  tech_support: {
    ...
    summary: {
      speedupZh: "快 20.3x (35ms vs 710ms 首字)",
      speedupEn: "20.3x faster (35ms vs 710ms TTFT)",
      costZh: "~$6.30 (Jev) vs ~$26.80 (LLM) · 省 76%",
      costEn: "~$6.30 (Jev) vs ~$26.80 (LLM) · 76% Cheaper"
    }
  },
  spam_promo: {
    ...
    summary: {
      speedupZh: "快 25.8x (24ms vs 620ms 首字)",
      speedupEn: "25.8x faster (24ms vs 620ms TTFT)",
      costZh: "~$6.30 (Jev) vs ~$23.20 (LLM) · 省 73%",
      costEn: "~$6.30 (Jev) vs ~$23.20 (LLM) · 73% Cheaper"
    }
  }
};
...
async function runBenchmark() {
  ...
  if (currentMode === 'replay') {
    ...
    var jevTicker = setInterval(function() {
      ...
      if (elapsed >= targetJevMs) {
        ...
        if (jevTimerEl) jevTimerEl.textContent = elapsed;
      }
    }, 10);
    ...
    var llmTicker = setInterval(function() {
      ...
      if (elapsed >= targetTtft) {
        clearInterval(llmTicker);
        if (llmTimerEl) llmTimerEl.textContent = targetTtft.toLocaleString();
      } else {
        if (llmTimerEl) llmTimerEl.textContent = elapsed.toLocaleString();
      }
    }, 20);
  ...
  var handleLiveJev = async function() {
    ...
    if (res && res.mode === 'live' && res.answers) {
      ...
      if (jevTimerEl) jevTimerEl.textContent = pureMs;
      ...
      if (jevCostVal) jevCostVal.textContent = '~$6.30';
      ...
      if (jevScoreVal) jevScoreVal.textContent = currentLang === 'zh' ? matchedData.jev.scoreZh : matchedData.jev.scoreEn;
      if (jevCostVal) jevCostVal.textContent = matchedData.jev.cost;
      return { pureMs: pureFall, e2e: 1180 };
    }
  };
  ...
  var handleLiveLlm = async function() {
      ...
      if (res && res.ok && (res.headers.get('content-type') || '').indexOf('text/event-stream') >= 0) {
        ...
        while (true) {
          ...
          if (!firstTokenTime) {
            ...
            if (llmTimerEl) llmTimerEl.textContent = ttftVal.toLocaleString();
          }
        ...
        llmLiveSuccess = true;
        if (llmTimerEl) llmTimerEl.textContent = ttft.toLocaleString();
        ...
        // Parse JSON output into verdicts
        try {
          var fullObj = JSON.parse(streamText.trim());
          if (llmCodeEl) llmCodeEl.textContent = JSON.stringify(fullObj, null, 2);
        ...
        if (llmCostVal) llmCostVal.textContent = matchedData.llm.cost;
        return { ttft: ttft, totalLlm: totalLlm };
      }
    } catch (e) {}
    ...
    if (llmSeverityVal) llmSeverityVal.textContent = currentLang === 'zh' ? matchedData.llm.severityZh : matchedData.llm.severityEn;
    if (llmCostVal) llmCostVal.textContent = matchedData.llm.cost;
    return { ttft: targetTtft, totalLlm: 1160 };
  };
  ...
  if (speedupEl) {
    speedupEl.textContent = currentLang === 'zh'
      ? ('快 ' + liveSpeedup + 'x (' + jRes.pureMs + 'ms vs ' + lRes.ttft + 'ms 首字)')
      : (liveSpeedup + 'x faster (' + jRes.pureMs + 'ms vs ' + lRes.ttft + 'ms TTFT)');
  }
