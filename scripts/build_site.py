...
<section id="live-bench" class="interactive-bench" aria-label="Live Benchmark">
  <div class="bench-header">
    ...
    <div id="bench-badge" class="bench-mode-badge" title="Runtime Mode">
      <span class="dot"></span>
      <span class="lang-zh" id="bench-mode-text-zh">探测连接...</span>
      <span class="lang-en" id="bench-mode-text-en" style="display:none">Probing API...</span>
    </div>
  </div>
...
function initBenchmark() {
  ...
  if (badge) {
    badge.addEventListener('click', function() {
      ...
      var promptMsg = currentLang === 'zh'
        ? '输入您的 OpenRouter API Key（仅保存在当前浏览器本地，直连模型）：'
        : 'Enter your OpenRouter API Key (stored locally in browser):';
...
async function checkApiStatus() {
  ...
  if (textZh) textZh.textContent = '探测连接...';
  if (textEn) textEn.textContent = 'Probing API...';
  if (badge) badge.classList.remove('is-offline');
  ...
  try {
    ...
    var res = await fetch('/api/decide', {
      method: 'POST',
      headers: headers,
      body: JSON.stringify({ state: 'ping' })
    });
    if (res.ok) {
      ...
      if (data.mode === 'live') {
        ...
        if (badge) {
          ...
          badge.title = currentLang === 'zh'
            ? (savedKey ? 'Live API 已激活（使用自定义Key，点击修改）' : 'Live API 已激活（使用服务端全局凭据）')
            : (savedKey ? 'Live API Active (Custom Key, click to edit)' : 'Live API Active (Server Credentials)');
        }
        if (textZh) textZh.textContent = savedKey ? 'Live API (自定义Key)' : 'Live API (已连接)';
        if (textEn) textEn.textContent = savedKey ? 'Live API (Custom Key)' : 'Live API (Connected)';
        return;
      }
    }
  } catch (e) {
    console.error('Decide probe error:', e);
  }
  ...
  if (badge) {
    ...
    badge.title = currentLang === 'zh'
      ? (savedKey ? 'Key 连接未成功（点击重新配置）' : '点击配置 OpenRouter Key 可直接连真机')
      : (savedKey ? 'Key connection failed (click to retry)' : 'Click to configure OpenRouter API Key');
  }
  if (textZh) textZh.textContent = savedKey ? '连接未就绪 (点击重试)' : '基准回放 (点击配置Key)';
  if (textEn) textEn.textContent = savedKey ? 'Not Ready (Click to retry)' : 'Trace Replay (Click to config)';
}
...
async function runBenchmark() {
  ...
  var targetJevPureMs = matchedData.jev.latency;
  var targetJevE2eMs = matchedData.jev.e2e;
  ...
  var ticker = setInterval(function() {
    ...
    if (!jevSettled) {
      jevTimerEl.textContent = Math.min(elapsed, targetJevPureMs);
    }
  }, 16);
  ...
  // Concurrent Execution: Jev
  var jevPromise = (apiAvailable !== false || savedKey)
    ? fetch('/api/decide', {
        method: 'POST',
        headers: reqHeaders,
  ...
  // Concurrent Execution: Gemini 2.5 Flash-Lite
  var llmPromise = (apiAvailable !== false || savedKey)
    ? fetch('/api/llm', {
        method: 'POST',
        headers: reqHeaders,
        body: JSON.stringify({ state: inputText })
  ...
  // Handle Jev
  var handleJev = async function() {
    var res = await Promise.race([
      jevPromise,
      new Promise(function(r) { setTimeout(r, 6500); })
    ]);
    ...
    if (res && res.mode === 'live' && res.answers) {
      ...
      // Fallback
      jevTimerEl.textContent = targetJevPureMs;
      ...
      if (jevMeta2Zh) jevMeta2Zh.textContent = '端到端物理链路: ' + targetJevE2eMs.toLocaleString() + 'ms (含跨洋公网RTT)';
      if (jevMeta2En) jevMeta2En.textContent = 'Physical RTT: ' + targetJevE2eMs.toLocaleString() + 'ms (Cross-Ocean)';
      return { pureMs: targetJevPureMs, e2e: targetJevE2eMs };
    }
  };

  // Handle Gemini 2.5 Flash-Lite Stream
  var handleLlm = async function() {
    try {
      var res = await Promise.race([
        llmPromise,
        new Promise(function(r) { setTimeout(r, 6500); })
    ...
    // Fallback animation for Gemini 2.5 Flash-Lite
    var targetLlmMs = matchedData.llm.e2e;
    ...
    // Wait for simulated TTFT arrival
    await new Promise(function(r) { setTimeout(r, targetTtft); });
    llmTimerEl.textContent = targetTtft.toLocaleString();

    // Stream out JSON
    var chars = targetJson.length;
    var idx = 0;
    var delay = Math.max(10, Math.floor(targetGenMs / chars));
    while (idx < chars) {
    ...
    return { ttft: targetTtft, genTime: targetGenMs, totalLlm: targetLlmMs };
  };

  // Run both
  var results = await Promise.all([handleJev(), handleLlm()]);
  clearInterval(ticker);

  var jevRes = results[0];
  var llmRes = results[1];

  var speedupRatio = (llmRes.ttft / jevRes.pureMs).toFixed(1);
  if (speedupEl) speedupEl.textContent = speedupRatio + 'x (TTFT比)';

  if (jevLiveSuccess && llmLiveSuccess) {
    if (srcZh) { srcZh.textContent = 'Live API (OpenRouter 双模型真机并发)'; srcZh.style.color = '#047857'; srcZh.style.fontWeight = '500'; }
    if (srcEn) { srcEn.textContent = 'Live API (Dual Model Concurrent)'; srcEn.style.color = '#047857'; srcEn.style.fontWeight = '500'; }
  } else if (jevLiveSuccess) {
    if (srcZh) { srcZh.textContent = 'Live Jev + Gemini 基准流'; srcZh.style.color = '#047857'; srcZh.style.fontWeight = '500'; }
    if (srcEn) { srcEn.textContent = 'Live Jev + Gemini Stream'; srcEn.style.color = '#047857'; srcEn.style.fontWeight = '500'; }
  } else {
    if (srcZh) { srcZh.textContent = '真机基准回放 (Gemini 2.5 Flash-Lite)'; srcZh.style.color = ''; srcZh.style.fontWeight = ''; }
    if (srcEn) { srcEn.textContent = 'Trace Replay (Gemini 2.5 Flash-Lite)'; srcEn.style.color = ''; srcEn.style.fontWeight = ''; }
  }
