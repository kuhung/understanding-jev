...
<section id="live-bench" class="interactive-bench" aria-label="Live Benchmark">
  ...
  <div class="bench-grid">
    ...
    <div class="bench-col jev-col">
      ...
      <div class="bench-latency-box">
        <div class="bench-latency-num">
          <span id="jev-timer">161</span><span class="bench-latency-unit">ms</span>
        ...
        <div class="bench-meta-tag green">
          <span>●</span>
          <span class="lang-zh" id="jev-meta-zh">Provider 161ms</span>
          <span class="lang-en" id="jev-meta-en" style="display:none">Provider 161ms</span>
        </div>
        <div class="bench-felt lang-zh" id="jev-felt-zh">网页体感 534ms</div>
        <div class="bench-felt lang-en" id="jev-felt-en" style="display:none">Page RTT 534ms</div>
      </div>
      <div class="bench-verdicts">
        <div class="verdict-row">
          <div class="verdict-label-row">
            ...
            <span id="jev-noul-val" class="verdict-val">高危拦截 (99%)</span>
          </div>
          <div class="primitive-bar-track">
            <div id="jev-noul-bar" class="primitive-bar-fill" style="width: 99%;"></div>
          </div>
        </div>
        <div class="verdict-row">
          <div class="verdict-label-row">
      ...
      <div class="bench-cost-tag">
        ...
        <strong id="jev-cost-val">$0.0000231</strong>
      </div>
    </div>
    ...
    <div class="bench-col llm-col">
      ...
      <div class="bench-latency-box">
        <div class="bench-latency-num">
          <span id="llm-timer">992</span><span class="bench-latency-unit">ms</span>
        ...
        <div class="bench-meta-tag orange">
          <span>●</span>
          <span class="lang-zh" id="llm-meta-zh">首字 819ms</span>
          <span class="lang-en" id="llm-meta-en" style="display:none">Provider 819ms</span>
        </div>
        <div class="bench-felt lang-zh" id="llm-felt-zh">网页体感 1352ms</div>
        <div class="bench-felt lang-en" id="llm-felt-en" style="display:none">Page RTT 1352ms</div>
      </div>
      <div class="bench-verdicts">
        <div class="verdict-row">
          <div class="verdict-label-row">
  ...
  "reason": "SQL injection tautology detected."
      ...
      <div class="bench-cost-tag">
        <span class="lang-zh">单次调用成本</span>
        <span class="lang-en" style="display:none">Cost / Call</span>
        <strong id="llm-cost-val">$0.0000456</strong>
      </div>
    </div>
  </div>
  ...
  <div class="bench-summary-bar">
    <div>
      ...
      <span id="summary-speedup">快 ~6.2x (161ms Provider vs 992ms 生成)</span>
    </div>
    <div>
      <strong class="lang-zh">单次成本差值：</strong>
      <strong class="lang-en" style="display:none">Cost Diff: </strong>
      <span class="lang-zh" id="summary-cost-zh">单次立省 $0.0000225 (降 49.4%)</span>
      <span class="lang-en" id="summary-cost-en" style="display:none">Save $0.0000225 / call (-49.4%)</span>
    </div>
    <div>
...
var BENCH_DATA = {
  sql_injection: {
    ...
    summary: {
      speedupZh: "快 ~6.2x (161ms Provider vs 992ms 生成)",
      speedupEn: "~6.2x faster (161ms Provider vs 992ms Generation)",
      costZh: "单次立省 $0.0000225 (降 49.4%)",
      costEn: "Save $0.0000225 / call (-49.4%)"
    }
  },
  angry_customer: {
    ...
    summary: {
      speedupZh: "快 ~3.6x (176ms Provider vs 638ms 生成)",
      speedupEn: "~3.6x faster (176ms Provider vs 638ms Generation)",
      costZh: "单次立省 $0.0000325 (降 58.2%)",
      costEn: "Save $0.0000325 / call (-58.2%)"
    }
  },
  tech_support: {
    ...
    summary: {
      speedupZh: "快 ~39.8x (166ms Provider vs 6600ms 生成)",
      speedupEn: "~39.8x faster (166ms Provider vs 6600ms Generation)",
      costZh: "单次立省 $0.0000292 (降 55.4%)",
      costEn: "Save $0.0000292 / call (-55.4%)"
    }
  },
  spam_promo: {
    ...
    summary: {
      speedupZh: "快 ~3.1x (213ms Provider vs 654ms 生成)",
      speedupEn: "~3.1x faster (213ms Provider vs 654ms Generation)",
      costZh: "单次立省 $0.0000324 (降 58.5%)",
      costEn: "Save $0.0000324 / call (-58.5%)"
    }
  }
};
...
function applyOpenRouterClock(prefix, or, feltMs, streamed) {
  ...
  var providerMs = or && or.provider_ms != null ? Math.round(or.provider_ms) : null;
  var generationMs = or && or.generation_ms != null ? Math.round(or.generation_ms) : null;
  ...
  if (!metaCoreZh.length) {
    if (streamed && providerMs != null) {
      metaCoreZh.push('首字 ' + providerMs + 'ms');
      metaCoreEn.push('Provider ' + providerMs + 'ms');
    } else if (providerMs != null) {
      metaCoreZh.push('Provider ' + providerMs + 'ms');
      metaCoreEn.push('Provider ' + providerMs + 'ms');
    } else if (generationMs != null) {
      metaCoreZh.push('生成 ' + generationMs + 'ms');
      metaCoreEn.push('Generation ' + generationMs + 'ms');
    }
  }
...
async function runBenchmark() {
  ...
  if (currentMode === 'replay') {
    ...
    var llmTickCap = Math.min(targetTtft, 900);
    ...
    var replayTicker = setInterval(function() {
      ...
      if (!jevReplayDone) {
        if (elapsed >= targetJevMs) {
          ...
          if (jevMetaZh) jevMetaZh.textContent = 'Provider ' + targetJevMs + 'ms';
          if (jevMetaEn) jevMetaEn.textContent = 'Provider ' + targetJevMs + 'ms';
          setFelt('jev', matchedData.jev.felt != null ? matchedData.jev.felt : targetJevMs);
        } else if (jevTimerEl) {
          jevTimerEl.textContent = elapsed;
        }
      }
      if (!llmReplayDone) {
        if (elapsed >= llmTickCap) {
          llmReplayDone = true;
          if (llmTimerEl) llmTimerEl.textContent = llmTickCap;
          setFelt('llm', matchedData.llm.felt != null ? matchedData.llm.felt : targetGenMs);
        } else if (llmTimerEl) {
    ...
    if (llmMetaZh) llmMetaZh.textContent = '首字 ' + targetTtft + 'ms';
    if (llmMetaEn) llmMetaEn.textContent = 'Provider ' + targetTtft + 'ms';
    setFelt('llm', matchedData.llm.felt != null ? matchedData.llm.felt : targetGenMs);
    ...
    clearInterval(replayTicker);
    setTypeLabel('jev', 'Provider', 'Provider');
    setTypeLabel('llm', '生成耗时', 'Generation');
    if (jevTimerEl) jevTimerEl.textContent = targetJevMs;
    if (llmTimerEl) llmTimerEl.textContent = targetGenMs;
  ...
  var handleLiveJev = async function() {
    ...
    if (res && res.mode === 'live' && res.answers) {
      ...
      if (jevScoreVal) jevScoreVal.textContent = currentLang === 'zh' ? matchedData.jev.scoreZh : matchedData.jev.scoreEn;
      if (jevCostVal) jevCostVal.textContent = matchedData.jev.cost;
      return { latency: pureFall, compute: pureFall, felt: e2e, cost: matchedData.jev.costNum || 0.000023 };
    }
  };
  ...
  var handleLiveLlm = async function() {
    ...
    if (runId !== currentRunId) return null;
    if (llmTimerEl) llmTimerEl.textContent = String(matchedData.llm.genMs);
    if (llmCodeEl) llmCodeEl.textContent = matchedData.llm.json;
    if (llmThreatVal) llmThreatVal.textContent = currentLang === 'zh' ? matchedData.llm.threatZh : matchedData.llm.threatEn;
    if (llmActionVal) llmActionVal.innerHTML = '<span class="badge-choice ' + matchedData.llm.actionClass + '">' + (currentLang === 'zh' ? matchedData.llm.actionZh : matchedData.llm.actionEn) + '</span>';
    if (llmSeverityVal) llmSeverityVal.textContent = currentLang === 'zh' ? matchedData.llm.severityZh : matchedData.llm.severityEn;
    if (llmCostVal) llmCostVal.textContent = matchedData.llm.cost;
    return { ttft: targetTtft, compute: matchedData.llm.genMs, felt: matchedData.llm.felt || targetTtft, cost: matchedData.llm.costNum || 0.0000456 };
  };
...
function switchLang(lang) {
  ...
  document.title = lang === 'zh'
    ? '深入解读 Jev 模型：架构拆解、RLCD 原理与工程实测'
    : 'Reading Jev: Architecture, RLCD, and Engineering Limits';
...
<title>深入解读 Jev 模型：架构拆解、RLCD 原理与工程实测</title>
<meta name="description" content="一线开发者拆解 Jev 单步决策模型：深入 RLCD 概率校准原理、Qwen 基座与生产架构设计，包含 20 行本地代码实测、开源生态复现及万次压测复盘。">
<meta name="keywords" content="Jev, Jev模型, Jev架构, Jev原理, RLCD, Qwen-2.5-1B-RLCD, 单步决策模型, Jev开源, Jev使用, 本地部署, 毫秒级判定">
<link rel="canonical" href="https://jev.kuhung.me/">
<meta property="og:type" content="article">
<meta property="og:url" content="https://jev.kuhung.me/">
<meta property="og:title" content="深入解读 Jev 模型：架构拆解、RLCD 原理与工程实测">
<meta property="og:description" content="一线开发者拆解 Jev 单步决策模型：深入 RLCD 概率校准原理、Qwen 基座与生产架构设计，包含 20 行本地代码实测、开源生态复现及万次压测复盘。">
<meta name="twitter:card" content="summary">
<meta name="twitter:title" content="深入解读 Jev 模型：架构拆解、RLCD 原理与工程实测">
<meta name="twitter:description" content="一线开发者拆解 Jev 单步决策模型：深入 RLCD 概率校准原理、Qwen 基座与生产架构设计，包含 20 行本地代码实测、开源生态复现及万次压测复盘。">
<script type="application/ld+json">
{{
  "@context": "https://schema.org",
  "@type": "TechArticle",
  "headline": "深入解读 Jev 模型：架构拆解、RLCD 原理与工程实测",
  "description": "一线开发者拆解 Jev 单步决策模型：深入 RLCD 概率校准原理、Qwen 基座与生产架构设计，包含 20 行本地代码实测、开源生态复现及万次压测复盘。",
  "url": "https://jev.kuhung.me/",
  ...
  "inLanguage": ["zh-CN", "en"],
  "keywords": "Jev, Jev模型, Jev架构, Jev原理, RLCD, Qwen-2.5-1B-RLCD, 单步决策模型, Jev开源, 本地实测"
}}
</script>
{VERCEL_OBSERVABILITY}
